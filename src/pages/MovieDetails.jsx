...
import { X, Calendar, Clapperboard, StickyNote, Monitor, ExternalLink, Star, Clock, Play, ArrowLeft, Loader2, Plus, Check, Users, Building, Quote, Wallet, BarChart3 } from "lucide-react";
    ...
    const handleAdd = async () => {
        ...
        try {
            ...
            toast({
                ...
                description: `"${movie.title}" added to your Plan to Watch list.`,
                variant: "success",
            });
        } catch (error) {
            console.error(error);
            ...
            <div className="relative">
                ...
                <div className="relative h-[40vh] sm:h-[50vh] w-full overflow-hidden">
                    ...
                    <div className="absolute bottom-0 left-0 w-full p-6 sm:p-10 flex flex-col sm:flex-row items-start sm:items-end gap-6 z-10">
                        ...
                        <div className="flex-1 min-w-0 space-y-2 mb-2 w-full">
                             <h1 className="text-3xl sm:text-4xl md:text-5xl font-bold text-white leading-tight drop-shadow-lg">{movie.title}</h1>
                             
                             {movie.tagline && (
                                <p className="text-lg text-neutral-300 italic flex items-center gap-2">
                                    <Quote size={16} className="text-neutral-500 fill-neutral-500" />
                                    {movie.tagline}
                                </p>
                             )}

                             <div className="text-lg sm:text-xl text-neutral-300 drop-shadow-md flex flex-wrap items-center gap-2">
                                {directors.length > 0 && <span>{directors.join(", ")}</span>}
                                {directors.length > 0 && movie.releaseDate && <span>•</span>}
                             ...
                             <div className="flex items-center gap-4 mt-4 sm:hidden w-full">
                                 ...
                                 <div className="flex-1 min-w-0">
                                     {isAdded ? (
                                        ...
                                        >
                                            {removing ? <Loader2 className="animate-spin" size={18} /> : <Check size={18} />}
                        ...
                        <div className="hidden sm:flex flex-col gap-3 shrink-0 mb-2">
                            {isAdded ? (
                                ...
                                >
                                    {removing ? <Loader2 className="animate-spin" size={20} /> : <Check size={20} />}
                                    {removing ? "Removing..." : "In Library"}
                                </button>
                             ) : (
                                <button 
                ...
                <div className="max-w-4xl mx-auto p-4 sm:p-8 space-y-8">
                    ...
                    <div className="flex flex-wrap gap-3">
                         {movie.tmdbId && (
                           <a 
                            href={movie.type === 'tv' ? `https://pstream.mov/media/tmdb-tv-${movie.tmdbId}` : `https://pstream.mov/media/tmdb-movie-${movie.tmdbId}`}
                            target="_blank"
                            rel="noreferrer"
                       ...
                       <a 
                        href={movie.imdbId ? `https://www.imdb.com/title/${movie.imdbId}` : `https://www.imdb.com/find?q=${encodeURIComponent(movie.title)}`}
                        target="_blank"
                        rel="noreferrer"
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                        <div className="bg-neutral-900 rounded-lg p-4 border border-neutral-800">
                         ...
                         <div className="bg-neutral-900 rounded-lg p-4 border border-neutral-800">
                            <div className="text-xs text-neutral-500 uppercase font-bold mb-1">Runtime</div>
                             <div className="flex items-center gap-2 text-white">
                                 <Clock size={18} className="text-purple-500"/>
                                 <span className="font-mono">{movie.runtime ? `${Math.floor(movie.runtime / 60)}h ${movie.runtime % 60}m` : 'N/A'}</span>
                            </div>
                        </div>

                         <div className="bg-neutral-900 rounded-lg p-4 border border-neutral-800">
                            <div className="text-xs text-neutral-500 uppercase font-bold mb-1">Status</div>
                            <div className="flex items-center gap-2 text-white">
                        ...
                        <div className="bg-neutral-900 rounded-lg p-4 border border-neutral-800">
                            <div className="text-xs text-neutral-500 uppercase font-bold mb-1">Type</div>
                             <div className="flex items-center gap-2 text-white">
                        ...
                        {movie.budget > 0 && (
                            <div className="bg-neutral-900 rounded-lg p-4 border border-neutral-800">
                                <div className="text-xs text-neutral-500 uppercase font-bold mb-1">Budget</div>
                                <div className="flex items-center gap-2 text-white">
                                    <Wallet size={18} className="text-emerald-500"/>
                                    <span className="font-mono">${(movie.budget / 1000000).toFixed(1)}M</span>
                                </div>
                            </div>
                        )}
                        ...
                        {movie.revenue > 0 && (
                            <div className="bg-neutral-900 rounded-lg p-4 border border-neutral-800">
                                <div className="text-xs text-neutral-500 uppercase font-bold mb-1">Revenue</div>
                                <div className="flex items-center gap-2 text-white">
                                    <BarChart3 size={18} className="text-indigo-500"/>
                                    <span className="font-mono">${(movie.revenue / 1000000).toFixed(1)}M</span>
                                </div>
                            </div>
                        )}
                    </div>
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {movie.cast.map((actor, i) => (
                                    <div key={i} className="bg-neutral-900 border border-neutral-800 rounded-full px-4 py-2 text-white flex items-center gap-2">
                    ...
                    {movie.productionCompanies && movie.productionCompanies.length > 0 && (
                        <div className="space-y-4">
                            ...
                            <div className="flex flex-wrap gap-2">
                                {movie.productionCompanies.map((company, i) => (
                                    <div key={i} className="bg-neutral-900 border border-neutral-800 rounded-full px-4 py-2 text-neutral-300 flex items-center gap-2">
                     ...
                     {movie.genres && movie.genres.length > 0 && (
                        <div className="space-y-4">
                            <h3 className="text-lg font-bold text-neutral-500 uppercase">Genres</h3>
                            <div className="flex flex-wrap gap-2">
                                {movie.genres.map(g => (
                                    <span key={g} className="px-3 py-1 rounded-full bg-neutral-800 text-neutral-300 text-sm border border-neutral-700">
                                        {g}
                                    </span>
                                ))}
                            </div>
                    ...
                    {availability.length > 0 && (
                        <div className="space-y-4">
                             <h3 className="text-lg font-bold text-neutral-500 uppercase">Available On</h3>
                            <div className="flex flex-wrap gap-3">
