...
export default function MovieCard({
    ...
    const getStatusIcon = (status) => {
        // TV Statuses
        if (status === "Watching")
            return <Play size={14} className="text-green-500 fill-green-500" />;
        if (status === "Completed")
    ...
    const availabilityList = Array.isArray(movie.availability)
        ? Array.from(
              new Set(
                  movie.availability.map(normalizeServiceName).filter(Boolean),
              ),
          )
        : [];
    ...
    const ratingScore = (() => {
        if (movie.ratings && movie.ratings.overall > 0)
            return movie.ratings.overall.toFixed(1);
        ...
        >
            ...
            <div className="absolute inset-0 p-3 flex flex-col justify-between">
                <div className="flex justify-between items-start">
                    ...
                    <div className="flex gap-1 items-center">
                        {/* Removed Quick Add from here */}
                        {movie.type === "tv" && (
                            <div
                                className="bg-black/50 p-1 rounded backdrop-blur-sm"
                                title="TV Show"
                            >
                            ...
                            <>
                                ...
                                {movie.voteAverage > 0 && (
                                    <div className="px-1.5 py-0.5 rounded bg-black/60 backdrop-blur-md border border-yellow-500/30 text-yellow-500 text-[10px] font-bold shadow-lg flex items-center gap-1">
                                        <span className="text-[8px] tracking-tighter font-black">
                                            IMDb
                                        </span>
                                        <span>
                ...
                <div className="space-y-1">
                    ...
                    {/* Quick Add Button (Mobile Friendly & Positioned at Bottom Right above footer) */}
                    {onAdd && !isAdded && !movie.status && (
                        <button
                            onClick={(e) => {
                                e.stopPropagation();
                                onAdd(movie);
                            }}
                            className="absolute bottom-2 right-2 bg-blue-600/90 backdrop-blur-md p-2 rounded-full text-white hover:bg-blue-500 hover:scale-110 transition-all z-30 shadow-lg md:opacity-0 md:group-hover:opacity-100"
                        >
                            <Plus size={12} />
                        </button>
                    )}
            ...
            {!readOnly && (
                <div className="absolute top-2 right-2 translate-x-10 opacity-0 transition-all duration-300 group-hover:translate-x-0 group-hover:opacity-100 flex flex-col gap-2">
                    ...
                    {movie.notes && (
                        <div
                            className="p-2 rounded-full bg-neutral-800 text-neutral-300 shadow-lg"
                            title="Has Notes"
                        >
                            <StickyNote size={14} />
                        </div>
