...
// half a star wide and holds everything up to its own value: 3.1-3.5 is the 3.5
// bucket. Rounding down instead would give 5.0 a bucket to itself and leave the
// other nine covering the wrong half-star.
//
...
export function ratingBucketIndex(score: number | null | undefined): number | null {
  ...
  return Math.min(RATING_BUCKET_COUNT, Math.max(1, Math.ceil(score * 2)));
}
